# 编译正式版联机 Hook：civ6hook.cpp -> build\hookdll.dll（x64，/MT 静态 CRT）
#
# 用法：在 PowerShell 里直接运行  .\build-hook.ps1
...
$compileLine = 'call "{0}" -arch=amd64 -no_logo && cd /d "{1}" && cl /nologo /utf-8 /LD /O2 /EHsc /W3 /MT /D_CRT_SECURE_NO_WARNINGS civ6hook.cpp /Fo:"{2}\\" /Fe:"{2}\hookdll.dll"' -f `
...
if (Test-Path $dll) {
    try {
        ...
        Write-Host "删不掉旧产物（$($_.Exception.Message)）—— 可能正被占用，继续尝试编译。" -ForegroundColor Yellow
    }
}
...
if ((Test-Path $dll) -and $exitCode -eq 0) {
    ...
    Write-Host "接下来运行 .\deploy-hook.ps1 把它装进启动器的自动注入路径。" -ForegroundColor Cyan
}
else {
